...
const RELEASE_BASE = `https://github.com/yancyuu/agentpanel/releases/download/${DESKTOP_VERSION}`;
const DOWNLOADS = {
  macArm: {
    label: 'macOS 客户端',
    shortLabel: 'macOS Apple Silicon',
    detail: 'Apple Silicon · DMG · 208 MB',
    filename: `AgentPanel-${DESKTOP_VERSION}-arm64.dmg`,
    href: `${RELEASE_BASE}/AgentPanel-${DESKTOP_VERSION}-arm64.dmg`,
    kind: 'desktop',
  },
  macIntel: {
    ...
    filename: `AgentPanel-${DESKTOP_VERSION}-x64.dmg`,
    href: `${RELEASE_BASE}/AgentPanel-${DESKTOP_VERSION}-x64.dmg`,
    kind: 'desktop',
  },
  windows: {
    label: 'Windows 客户端',
    ...
    filename: `AgentPanel-Setup-${DESKTOP_VERSION}-x64.exe`,
    href: `${RELEASE_BASE}/AgentPanel-Setup-${DESKTOP_VERSION}-x64.exe`,
    kind: 'desktop',
  },
  linux: {
    label: 'Linux 客户端',
    shortLabel: 'Linux x64',
    detail: 'Linux x64 · AppImage · 142 MB',
    filename: `AgentPanel-${DESKTOP_VERSION}-x86_64.AppImage`,
    href: `${RELEASE_BASE}/AgentPanel-${DESKTOP_VERSION}-x86_64.AppImage`,
    kind: 'desktop',
  },
  windowsArm: {
    ...
    filename: `AgentPanel-Setup-${DESKTOP_VERSION}-arm64.exe`,
    href: `${RELEASE_BASE}/AgentPanel-Setup-${DESKTOP_VERSION}-arm64.exe`,
    kind: 'desktop',
  },
  linuxArm: {
    label: 'Linux ARM64 客户端',
    shortLabel: 'Linux ARM64',
    detail: 'Linux ARM64 · AppImage · 138 MB',
    filename: `AgentPanel-${DESKTOP_VERSION}-arm64.AppImage`,
    href: `${RELEASE_BASE}/AgentPanel-${DESKTOP_VERSION}-arm64.AppImage`,
    kind: 'desktop',
  },
};
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .trust-grid { width: min(1180px, calc(100% - 48px)); margin: 0 auto; display: grid; grid-template-columns: repeat(4, 1fr); }
    ...
    .all-downloads { display: grid; grid-template-columns: repeat(4, 1fr); border-top: 1px solid rgba(255,255,255,.13); }
    .all-downloads a { padding: 18px 20px; border-right: 1px solid rgba(255,255,255,.13); }
    .all-downloads a:last-child { border-right: 0; }
    ...
    @media (max-width: 980px) {
      ...
      .all-downloads a:nth-child(2) { border-right: 0; }
      .all-downloads a:nth-child(-n+2) { border-bottom: 1px solid rgba(255,255,255,.13); }
    }
...
<body>
  <header class="site-header">
    <nav class="nav" aria-label="主导航">
      ...
      <div class="nav-links">
        ...
        <a href="#advanced">高级用户</a>
        <a class="nav-download" href="#download">下载客户端</a>
      </div>
    </nav>
  </header>
  ...
  <main id="top">
    <section class="hero">
      <div class="hero-inner">
        <span class="eyebrow"><span class="eyebrow-dot"></span> 本地优先 · Desktop v${DESKTOP_VERSION}</span>
        <h1>让智能体，<br />真正把工作做完。</h1>
        <p class="hero-lede">不用学编程，也不用守着终端。创建任务、选择智能体或小队、查看进度、审核成果——全部在一个本地客户端里完成。</p>
        <div class="hero-actions">
          <a id="heroDownload" class="button button-primary" href="${DOWNLOADS.macArm.href}"><span aria-hidden="true">↓</span><span id="heroDownloadText">下载 macOS 客户端</span></a>
          <a class="button button-secondary" href="#product">看看它能做什么 <span aria-hidden="true">→</span></a>
        </div>
        <div id="heroDownloadContext" class="download-context">正在识别你的系统…</div>
        <div class="works-with">
          <span>可连接</span>
          <strong>Claude Code</strong><span class="runtime-dot">•</span>
          <strong>Codex</strong><span class="runtime-dot">•</span>
          <strong>Gemini CLI</strong><span class="runtime-dot">•</span>
          <strong>Cursor</strong><span class="runtime-dot">•</span>
    ...
    <section id="product" class="section feature-split">
      <div class="feature-copy">
        <span class="section-kicker">任务与收件箱</span>
        <h2>像交代同事一样，把任务交给智能体。</h2>
        <p class="section-intro">写清楚想要的结果，附上图片、PDF、Office 或 ZIP 文件，然后等待交付。过程状态和需要你处理的反馈会统一进入收件箱。</p>
        <div class="feature-points">
          <div class="feature-point"><span class="feature-icon">1</span><div><strong>不需要写 Prompt 工程</strong><p>用普通语言描述目标，选择合适的智能体即可。</p></div></div>
          <div class="feature-point"><span class="feature-icon">2</span><div><strong>输入和成果分开管理</strong><p>原始材料进入 input，正式成果按版本归档到 outputs。</p></div></div>
    ...
    <section class="section feature-split reverse">
      <div class="feature-copy">
        <span class="section-kicker">智能体小队</span>
        <h2>复杂任务，让一支小队协作完成。</h2>
        <p class="section-intro">成员先进行有限轮次圆桌讨论，自行选出本任务的临时队长。队长负责分工、验收、整合，并向你统一交付最终结果。</p>
        <div class="feature-points">
          <div class="feature-point"><span class="feature-icon">✓</span><div><strong>任务级临时队长</strong><p>不是固定上下级，每次任务都能选择最合适的负责人。</p></div></div>
          <div class="feature-point"><span class="feature-icon">↗</span><div><strong>并行执行，集中交付</strong><p>成员同步工作，队长最终整合，避免你逐个追进度。</p></div></div>
    ...
    <section id="workflow" class="section workflow-section">
      ...
      <div class="workflow-grid">
        ...
        <article class="workflow-card"><div class="workflow-number">02</div><h3>自动执行</h3><p>智能体自行推进；小队会先讨论、选队长，再并行分工。</p></article>
        <article class="workflow-card"><div class="workflow-number">03</div><h3>收到交付</h3><p>成果进入收件箱，报告、图片和附件可以直接查看。</p></article>
        <article class="workflow-card"><div class="workflow-number">04</div><h3>审核归档</h3><p>满意则保存正式版本；需要修改则在原任务继续返工。</p></article>
      </div>
    </section>
    ...
    <section id="download" class="download-section">
      <div class="download-inner">
        ...
        <div class="download-card">
          ...
          <div id="recommendedDetail" class="download-detail">Apple Silicon · DMG · 208 MB</div>
      ...
      <div class="all-downloads">
        ...
        <a href="${DOWNLOADS.macIntel.href}"><strong>macOS Intel</strong><span>ZIP 独立包</span></a>
        <a href="${DOWNLOADS.windows.href}"><strong>Windows x64</strong><span>ZIP 独立包</span></a>
        <a href="${DOWNLOADS.linux.href}"><strong>Linux x64</strong><span>ZIP 独立包</span></a>
      </div>
    </section>
    ...
    <section id="advanced" class="advanced">
      <details class="advanced-card">
        ...
        <div class="advanced-body">
          ...
          <p>可以。客户端本身已经内置 AgentPanel，首次启动后会创建 <code>~/.hermit/bin/agentpanel</code>，App 和终端共用同一份任务与配置数据，不需要再次执行 npm 安装。</p>
          <div class="command-box"><code>~/.hermit/bin/agentpanel --version</code><button class="copy-button" data-copy="~/.hermit/bin/agentpanel --version">复制</button></div>
          <p><strong>直接输入 <code>agentpanel</code>：</strong>客户端首次启动会在现有 PATH 中寻找可写的标准命令目录，并安全创建指向内置 CLI 的命令链接；不会覆盖同名用户命令，也不会修改 Shell 配置。若系统目录不可写，仍可使用上面的完整路径。</p>

          <h3>不安装桌面 App</h3>
          <p>传统 CLI 安装只推荐给需要脚本化或服务器环境的高级用户。</p>
          <div class="command-box"><code>curl -fsSL https://gh-proxy.com/https://raw.githubusercontent.com/yancyuu/agentpanel/master/scripts/install.sh | bash</code><button class="copy-button" data-copy="curl -fsSL https://gh-proxy.com/https://raw.githubusercontent.com/yancyuu/agentpanel/master/scripts/install.sh | bash">复制</button></div>
          <div class="command-box"><code>npm install -g @yancyyu/agentpanel</code><button class="copy-button" data-copy="npm install -g @yancyyu/agentpanel">复制</button></div>

          <h3>常用命令</h3>
          <ul>
            <li><code>agentpanel --version</code>：查看当前版本。</li>
            <li><code>agentpanel tasks list --team &lt;team&gt;</code>：查看团队任务。</li>
            <li><code>agentpanel doctor</code>：执行只读诊断。</li>
            <li><code>agentpanel usage status</code>：查看用量采集状态。</li>
          </ul>
          <p><a href="https://github.com/yancyuu/agentpanel">在 GitHub 查看完整说明 →</a></p>
        </div>
      </details>
    </section>
    ...
    <section class="faq">
      ...
      <details><summary>为什么终端里暂时找不到 agentpanel？</summary><p>客户端会创建 <code>~/.hermit/bin/agentpanel</code>，并在已有 PATH 的可写标准目录中创建安全链接。如果系统没有可写目录或已有同名用户命令，客户端不会覆盖；此时可直接运行完整路径。</p></details>
  ...
  <footer class="footer">
    <div class="footer-inner">
      ...
      <div class="footer-links"><a href="#download">下载</a><a href="#advanced">高级用户</a><a href="https://github.com/yancyuu/agentpanel">GitHub</a><span>© 2026 AgentPanel</span></div>
    </div>
  </footer>
  ...
  <script>
    ...
    function updateDownload(key) {
      ...
      document.getElementById('heroDownloadContext').textContent = '已识别：' + item.shortLabel + ' · ' + item.detail;
    }
    ...
    async function detectPlatform() {
      ...
      if (key === 'macArm' && navigator.userAgentData && navigator.userAgentData.getHighEntropyValues) {
        try {
          const values = await navigator.userAgentData.getHighEntropyValues(['architecture', 'bitness']);
          if (String(values.architecture).toLowerCase().includes('x86')) key = 'macIntel';
        } catch (_) {
...
copyFile('docs/screenshots/agentpanel/workbench/01-inbox.png', 'images/workbench-inbox.png');
copyFile('docs/screenshots/agentpanel/workbench/03-tasks.png', 'images/workbench-tasks.png');
copyFile('docs/screenshots/agentpanel/workbench/05-assistants.png', 'images/workbench-agents.png');
