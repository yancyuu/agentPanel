...
const DESKTOP_DMG_URL = `https://github.com/yancyuu/agentcli/releases/download/${DESKTOP_VERSION}/AgentCLI-${DESKTOP_VERSION}-arm64.dmg`;
...
<head>
  ...
  <title>AgentCLI — 本地 AI 助手桌面客户端</title>
  <meta name="description" content="安装一个 AgentCLI 桌面客户端，即可在本地管理智能体、任务、小队、定时任务和交付成果。内置 AgentCLI 与 cc-connect，无需预装 Node.js。" />
  <meta property="og:title" content="AgentCLI — 本地 AI 助手桌面客户端" />
  <meta property="og:description" content="面向普通用户的本地 AI 助手客户端。安装 App 即可使用完整工作台，同时保留内置 CLI 能力。" />
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .desktop-download { max-width: 600px; margin: 0 auto 18px; padding: 24px; border: 1px solid rgba(139, 92, 246, 0.45); border-radius: 14px; background: linear-gradient(145deg, rgba(139,92,246,0.14), var(--bg-card)); text-align: left; }
    .desktop-download h2 { margin: 0 0 6px; font-size: 20px; }
    .desktop-download p { margin: 0 0 18px; color: var(--text-muted); font-size: 14px; }
    .download-button { display: flex; align-items: center; justify-content: center; width: 100%; min-height: 48px; border-radius: 10px; background: var(--accent); color: white; font-size: 15px; font-weight: 700; }
    .download-button:hover { background: #7c3aed; text-decoration: none; }
    .download-meta { margin-top: 10px; color: var(--text-dim); font-size: 12px; text-align: center; }
    .cli-secondary-label { max-width: 600px; margin: 24px auto 10px; color: var(--text-muted); font-size: 13px; text-align: left; }
...
<body>
  ...
  <section class="hero">
    <span class="hero-badge">macOS 桌面客户端 · Local-first · v${PKG_VERSION}</span>
    <h1>AgentCLI</h1>
    <p class="lede">给普通用户使用的本地 AI 助手客户端。安装一个 App，即可管理智能体、任务、小队、收件箱、定时任务和交付成果。</p>
    <p class="oss-line">安装包已内置 AgentCLI、Node 运行能力和 cc-connect；无需预装 Node.js、pnpm 或全局 CLI，数据默认保存在本机 <code>~/.hermit/</code>。</p>

    <div class="desktop-download" id="download">
      <h2>推荐：安装桌面客户端</h2>
      <p>适用于 Apple Silicon Mac。下载 DMG 后拖入“应用程序”即可使用；客户端启动后也会提供内置 AgentCLI 命令能力。</p>
      <a class="download-button" href="${DESKTOP_DMG_URL}">下载 AgentCLI for macOS</a>
      <div class="download-meta">macOS arm64 · DMG 安装包 · v${DESKTOP_VERSION}</div>
    </div>
    <div class="install-help">
      <strong>为什么推荐客户端？</strong> 不需要终端、不需要 npm，打开 App 即可使用完整工作台。高级用户仍可通过客户端内置的 <code>~/.hermit/bin/agentcli</code> 使用命令行。
    </div>

    <p class="cli-secondary-label"><strong>高级用户：</strong>如果暂时不使用桌面客户端，也可以选择传统 CLI 安装方式。</p>
...
npm uninstall -g @yancyyu/agentcli</code></pre>
    ...
    <div class="tiers">
      <div class="tier">
        ...
        <ul>
          ...
          <li>token 池认领：从授权池签发一次性网关 key，可写入 Codex / Claude Code / Pi；支持配置快照与一键恢复</li>
          <li>支持自托管、可二次开发</li>
        </ul>
  ...
  <section id="config" class="prose">
    ...
    <p class="section-sub">AgentCli 读写的本机配置位置，以及如何认领网关 key 并写进 Codex / Claude Code / Pi。</p>
    ...
    <h3 id="token-claim">Token 池认领是什么？</h3>
    <div class="callout success">
      <div class="callout-title">这里的 token 不是“用量 token”</div>
      <p>Token 池认领指的是：AgentCli 为已授权用户从 AI 网关签发一个<strong>一次性明文访问 key</strong>，再把 key、网关地址和所选模型写入本机运行时配置。认领完成后，Codex、Claude Code 或 Pi 就可以通过统一网关调用已分配的模型。</p>
    </div>

    <h3>如何认领</h3>
    <ol>
      <li><span class="step-label">先登录</span> — 运行 <code>agentcli auth login</code>，再用 <code>agentcli auth status</code> 确认已登录。</li>
      <li><span class="step-label">进入认领入口</span> — 运行 <code>agentcli</code>，展开「token 池（Beta）」并选择「认领」。</li>
      <li><span class="step-label">选择运行时和模型</span> — 可选择 Codex、Claude Code、Pi 或组合；默认只选 Codex。随后从网关授权的模型中选择一个模型。</li>
      <li><span class="step-label">等待自动配置</span> — AgentCli 自动完成签发、领取一次性 key、备份原配置和写入配置；完成后重启所选运行时，并新开终端让系统环境变量生效。</li>
    </ol>

    <h3>认领后会改哪些配置？</h3>
    <ul>
      <li><strong>Claude Code</strong>：更新 <code>~/.claude/settings.json</code>，写入 <code>ANTHROPIC_BASE_URL</code>、<code>ANTHROPIC_AUTH_TOKEN</code> 和所选模型对应的 tier 配置，同时保留其它设置。</li>
      <li><strong>Codex</strong>：更新 <code>~/.codex/auth.json</code> 的 <code>OPENAI_API_KEY</code>，并在 <code>~/.codex/config.toml</code> 写入网关 provider、模型与 <code>responses</code> 协议；原有 <code>[projects.*]</code> 等其它区块会保留。</li>
      <li><strong>Pi</strong>：更新 <code>~/.pi/agent/auth.json</code>、<code>models.json</code> 和 <code>settings.json</code> 中的 <code>skg</code> provider、可用模型和默认模型。</li>
      <li><strong>通用环境</strong>：写入权限为 0600 的 <code>~/.hermit/aikey.env</code>；同时一次性更新 macOS/Linux shell rc 管理块或 Windows 当前用户环境变量。不会安装每次提示符都执行的 shell hook。</li>
    </ul>
    <div class="callout">
      <div class="callout-title">什么时候生效？</div>
      <p>运行时自己的配置通常在<strong>重启 Codex / Claude Code / Pi</strong> 后生效；系统环境变量需要新开终端。外部 agent 或手动脚本也可以运行 <code>source ~/.hermit/aikey.env</code>。Claude Code 使用 <code>ANTHROPIC_AUTH_TOKEN</code>，Codex 使用 <code>OPENAI_API_KEY</code>，不要混用变量名。</p>
    </div>
    <div class="callout warn">
      <div class="callout-title">安全与恢复</div>
      <p>每次认领前都会在 <code>~/.hermit/agentcli.env.bak</code> 新建一个带时间点的配置快照，最多保留最近 20 份；可从「token 池 → 恢复配置快照」选择任意时间点还原。一次性 key 只在领取时进入内存并写入你选择的本地配置，结果页只显示脱敏值；请勿分享相关配置文件或备份。该能力需要服务端授权，部分账户可能暂未开放。</p>
    </div>
  </section>
...
agentcli doctor</code></pre>
  ...
  <section id="faq" class="prose">
    ...
    <div class="faq-item">
      <div class="faq-q">Q：安装桌面客户端后还能使用 CLI 吗？</div>
      <p>可以。客户端已经内置 AgentCLI，并在启动后维护 <code>~/.hermit/bin/agentcli</code>。App 内部会直接使用它；终端用户可以运行该完整路径，或在设置中按提示把 <code>~/.hermit/bin</code> 加入 PATH。客户端不会静默修改你的 Shell 配置。</p>
    </div>
...
# 问确认时输 Y，然后重开 PowerShell 窗口</code></pre>
  ...
  <footer class="footer">
    ...
    <div class="footer-links">
      <a href="${DESKTOP_DMG_URL}">下载 macOS 客户端</a>
      <a href="https://www.npmjs.com/package/@yancyyu/agentcli">npm（高级用户）</a>
    </div>
  </footer>
