...
function platformResourcesDirectory(context) {
  if (context.electronPlatformName === 'darwin') {
    ...
    return {
      appBundle,
      resourcesDirectory: path.join(appBundle, 'Contents', 'Resources', 'agentcli'),
    };
  }
  return {
    appBundle: null,
    resourcesDirectory: path.join(context.appOutDir, 'resources', 'agentcli'),
  };
}
...
module.exports = async function desktopAfterPack(context) {
  const source = path.join(context.packager.projectDir, 'desktop-runtime', 'agentcli', 'node_modules');
  ...
  if (!existsSync(path.join(vendorRoot, keepTarget))) {
    throw new Error(`Desktop runtime is missing bundled cc-connect target: ${keepTarget}`);
  }
