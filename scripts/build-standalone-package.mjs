...
//
// Assemble a "no Node install" distribution of agentpanel into dist-standalone/:
// the app + a bundled portable node + a launcher. Users unzip and run
// `agentpanel.cmd` (Windows) / `agentpanel` (unix) — Node does NOT need to be
...
function writeLauncher() {
  if (isWin) {
    writeFileSync(
      path.join(OUT, 'agentpanel.cmd'),
      '@echo off\r\n"%~dp0node.exe" "%~dp0bin\\hermit.mjs" %*\r\n',
    );
  } else {
    const p = path.join(OUT, 'agentpanel');
...
async function main() {
  ...
  log('run it with:  .\\dist-standalone\\agentpanel.cmd   (Windows)');
}
