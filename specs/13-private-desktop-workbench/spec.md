...
将当前包含在 `agentpanel` npm 包中的 Web 工作台完整迁移为独立的 Windows/macOS 桌面应用，并把 CLI 收缩为消息总线、用户/鉴权和 token 池三个领域。第一版沿用当前 UI 风格，核心交互改为私人动态：一个任务是一条动态，用户在动态下评论即发起下一轮智能体执行。

## Problem Statement

当前 `agentpanel` 同时承担 CLI、Fastify 工作台 server、React renderer、cc-connect 运行时接入和 usage telemetry，导致：
...
工作台必须作为独立 Electron 应用构建和分发，不再作为 `agentpanel web` 或 CLI 交互菜单中的 Web 工作台启动。
