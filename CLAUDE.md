...
and not the separate AgentPanel / AgentBus `auth login` token.
...
bot/app token or AgentPanel auth token, and retain the complete personal scope
set.
...
The active team slug and AgentPanel port are runtime values. Never hardcode a test team, user, or port in operational instructions.
