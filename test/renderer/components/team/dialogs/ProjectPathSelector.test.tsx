  ...
  api: {
    config: {
      browseFolders: vi.fn(async (dirPath?: string) => ({
        path: dirPath || '/tmp/agentpanel-empty-ws',
        dirs: [],
      })),
    },
  },
  ...
  act(() => {
    root.render(
      <ProjectPathSelector
        ...
        customCwd="/tmp/agentpanel-empty-ws"
...
describe('FolderBrowser 空目录手动输入', () => {
  ...
  it('空目录时可手动输入路径并确认选择', async () => {
    ...
    setInputValue(pathInput, '/tmp/agentpanel-empty-ws');
    ...
    expect(onChange).toHaveBeenCalledWith('/tmp/agentpanel-empty-ws');
  });
});
