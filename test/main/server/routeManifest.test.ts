...
describe('server route manifest baseline', () => {
  ...
  it('pins critical wildcard, static, SSE and compatibility routes', () => {
    ...
    expect(keys.has('GET /api/external-channels')).toBe(true);
    expect(keys.has('PUT /api/external-channels/cc-connect')).toBe(true);
    ...
    expect(keys.has('GET /api/telemetry/conversations/export')).toBe(false);
    expect(keys.has('GET /api/telemetry/conversations/:sessionId')).toBe(false);
    ...
    expect(keys.has('GET /api/github-delivery/bindings')).toBe(true);
    expect(keys.has('POST /api/github-delivery/publish')).toBe(true);
  });
  ...
  it('records inline and composition ordering that extraction must preserve semantically', () => {
    ...
    expect(WORKBENCH_SOURCE).not.toContain('registerUsageTelemetryRoutes(app');
    expect(WORKBENCH_SOURCE).not.toContain('registerConversationTelemetryRoutes(app');
