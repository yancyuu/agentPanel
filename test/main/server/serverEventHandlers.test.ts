...
function createHarness() {
  ...
  const appendMessage = vi.fn((): Promise<GroupMessage> => Promise.resolve(PERSISTED_MESSAGE));
  const broadcastSse = vi.fn();
  const resolveTeamFromBridgeMessage = vi.fn(() => Promise.resolve('team-a'));
...
describe('server event handlers', () => {
  ...
  it('ignores cc-connect stream deltas and refreshes only when the reply completes', async () => {
    const harness = createHarness();
    harness.register();

    for (const delta of ['一', '次', '不', '必', '要', '的', '刷', '新']) {
      harness.bridge.emit('reply_stream', {
    ...
    expect(harness.resolveTeamFromBridgeMessage).not.toHaveBeenCalled();
