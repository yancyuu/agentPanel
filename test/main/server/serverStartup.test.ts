...
function createDependencies() {
  ...
  const dependencies = {
    ...
    ensureAgentPanelShim: vi.fn(() => {
      calls.push('agentpanel.ensure');
      return Promise.resolve();
    }),
    ccConnectEnabled: true,
    markBridgeBinaryCheck: vi.fn(),
    markBridgeLaunch: vi.fn(),
    markBridgeDisabled: vi.fn(),
    processTarget,
...
describe('standalone server startup', () => {
  ...
  it('starts the local desktop runtime without touching cc-connect when the external channel plugin is disabled', async () => {
    const { calls, dependencies, lifecycle } = createDependencies();

    await startStandaloneServerRuntime({ ...dependencies, ccConnectEnabled: false });
    ...
    expect(dependencies.bridgeLauncher.ensureBinaryReady).not.toHaveBeenCalled();
    expect(dependencies.bridgeLauncher.ensureRunning).not.toHaveBeenCalled();
    expect(dependencies.bridge.start).not.toHaveBeenCalled();
    expect(dependencies.imLiveWatcher.start).not.toHaveBeenCalled();
    expect(dependencies.markBridgeDisabled).toHaveBeenCalledOnce();
