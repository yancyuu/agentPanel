...
// These paths are constructed from the feature contract rather than string
// literals, so the TypeScript syntax scan below cannot discover them directly.
const GITHUB_DELIVERY_ROUTE_REGISTRATIONS: RouteRegistration[] = [
  ['GET', '/api/github-delivery/bindings'],
  ['PUT', '/api/github-delivery/bindings/:agentName'],
  ['DELETE', '/api/github-delivery/bindings/:agentName'],
  ['GET', '/api/github-delivery/archive/:teamName/:taskId'],
  ['GET', '/api/github-delivery/archive/:teamName/:taskId.zip'],
  ['POST', '/api/github-delivery/publish'],
].map(([method, path]) => ({
  file: 'src/features/github-delivery/main/adapters/input/registerGitHubDeliveryRoutes.ts',
  method,
  path,
  line: 0,
}));
...
export function collectRouteRegistrations(): RouteRegistration[] {
  return [
    ...ROUTE_SOURCE_PATHS.flatMap((sourcePath) => {
      const sourceText = readFileSync(sourcePath, 'utf8');
    ...
    ...GITHUB_DELIVERY_ROUTE_REGISTRATIONS,
  ];
}
