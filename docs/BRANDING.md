...
- 产品展示名：**AgentCLI**
- CLI 命令：`agentcli`
- npm 包：`@yancyyu/agentcli`
- GitHub 仓库：`yancyuu/agentcli`

新增的页面标题、导航、错误提示、日志说明、Prompt 注入和当前产品文档统一使用 `AgentCLI`。不要再以 `Hermit`、`openHermit` 或 `AgentCli` 作为用户可见产品名称。

## 内置运行时

AgentCLI 是 Workbench 内置的 Agent 运行时与协议客户端。用户不需要单独全局安装 AgentCLI；Web 服务或桌面应用负责提供与 Workbench 同版本的 CLI，并向其启动的 Agent 子进程注入受管 PATH。

Loop、任务看板和团队协作是建立在 AgentCLI 命令与 Workbench API 之上的功能模块。Agent 通过普通 Shell 调用 `agentcli ...`，不使用 MCP、Skills 或 Harness 原生任务工具维护协作状态。
...
新增代码应优先使用品牌中立或 AgentCLI 命名；旧标识只用于兼容读取、迁移、停止旧进程和解析历史数据。

## 迁移原则

1. 用户可见品牌直接统一为 AgentCLI。
