...
本文记录 `yancyuu/agentpanel` 当前发布流程。当前包名是 `@yancyyu/agentpanel`，版本事实以根目录 `package.json` 为准。

## 当前发布事实

| 项 | 当前值 |
|:---|:---|
| Repository | `https://github.com/yancyuu/agentpanel` |
| npm package | `@yancyyu/agentpanel` |
| CLI binary | `agentpanel` |
...
Windows standalone ZIP（含内置 node 与 agentpanel 可执行文件）**当前未做代码签名**：首次运行时 SmartScreen 可能提示「Windows 已保护你的电脑」。引导用户点「更多信息 → 仍要运行」即可；也可以先解压到用户目录（如 `%USERPROFILE%\Tools`）再运行。后续如需消除提示，再评估引入签名证书与 signtool 流程。
...
```bash
npm view @yancyyu/agentpanel version
npx @yancyyu/agentpanel@latest --version
```
...
写渠道能力时必须区分：AgentPanel 控制面能力、cc-connect 平台适配能力、目标能力。不要把平台列表写成 AgentPanel 内置 Bot。
...
```bash
gh run list --repo yancyuu/agentpanel --workflow release.yml --limit 3
gh release view <VERSION> --repo yancyuu/agentpanel
```
...
```bash
gh release delete <VERSION> --repo yancyuu/agentpanel --yes
git tag -d <VERSION>
