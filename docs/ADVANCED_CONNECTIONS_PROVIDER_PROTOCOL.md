...
高级连接不是某一个 AgentBus Host 的专用配置，而是 AgentPanel 连接外部服务的开放能力。

用户可以：

1. 在客户端“设置 → 高级”中填写一个服务地址。
2. AgentPanel 读取该服务公开的能力声明。
3. 根据声明展示可用的登录方式、团队总线、数据上报和 Token 池能力。
4. 用户逐项授权本地数据范围。
5. AgentPanel 只把已明确授权的数据发送给该连接。

当前 AgentBus + 飞书登录只是第一个兼容实现。未来服务方可以实现本文定义的接口，接收 AgentPanel 的本地数据或提供团队协作能力，而不需要修改 AgentPanel 核心业务。
