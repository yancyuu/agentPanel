...
export function resolveDesktopRuntimePaths(options: {
  ...
  const runtimeRoot = options.isPackaged
    ? path.join(options.resourcesPath, 'agentpanel')
    : options.appPath;
  return {
    ...
    cliEntry: path.join(runtimeRoot, 'bin', 'agentpanel.mjs'),
  };
}
...
export function buildDesktopServerEnvironment(options: {
  ...
  return {
    ...
    AGENTPANEL_DESKTOP: '1',
    AGENTPANEL_PACKAGED_DESKTOP: '1',
    AGENTPANEL_DESKTOP_SESSION_TOKEN: options.sessionToken,
    AGENTPANEL_PACKAGE_ROOT: options.runtimeRoot,
