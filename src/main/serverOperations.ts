...
} from './services/agentpanel/workbenchRuntimeEnv';
import {
...
import type { SystemManagerSummary } from '@shared/types/team';
...
export function createServerOperations({
  ...
  const restartBridge = async (): Promise<void> => {
    ...
    if (!managementReady) {
      ...
      const configBackup = `${environment.hermitBridgeConfigFile}.agentpanel-bak`;
      try {
    ...
    await runAdminLoopInit({
      ...
      hasExistingBootstrap: async () => {
        try {
          ...
          ).includes(AGENTPANEL_OPS_GUIDE_MARKER);
        } catch {
          return false;
        }
      },
      ...
      fetchGuide: async () => ({ statusCode: 200, body: CURRENT_AGENTPANEL_OPERATIONS_GUIDE }),
  ...
  const initializeTelemetryFromSettings = async (): Promise<void> => {
    // Usage collection and reporting are intentionally disabled. Keep the
    // lifecycle hook as a no-op so standalone startup remains backward compatible.
  };
  ...
  const resolveTeamFromBridgeMessageWithRetry = async (
    ...
    logger.warn(
      { sessionKey },
      'external platform bridge message could not be mapped to an AgentPanel team slug'
    );
    return null;
  };
