...
function isTransientWorkerCommand(command: string): boolean {
  return ['--scan-once', '--startup-once'].some((flag) => command.includes(flag));
}
 ...
 */
function signalUsageWorkers(selfPid: number, sig: 'SIGTERM' | 'SIGKILL'): void {
...
export async function reapOtherUsageWorkers(selfPid = process.pid): Promise<void> {
  signalUsageWorkers(selfPid, 'SIGTERM');
  await new Promise((resolve) => setTimeout(resolve, REAP_GRACE_MS));
  signalUsageWorkers(selfPid, 'SIGKILL');
}

/** Stop legacy Usage telemetry workers without modifying any local session data. */
export const stopUsageTelemetryWorkers = reapOtherUsageWorkers;
