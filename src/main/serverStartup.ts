...
export async function startStandaloneServerRuntime<TBridgeClient>({
  ...
  try {
    await ensureAgentPanelShim();
  } catch (error) {
    app.log.warn({ err: error }, 'AgentPanel Workbench shim provisioning failed');
  }
  if (ccConnectEnabled) {
