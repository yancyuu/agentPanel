...
import { provisionAgentPanelShim } from './services/agentpanel/AgentPanelShimProvisioner';
import { provisionDesktopCommandAliases } from './services/agentpanel/DesktopCommandAliasProvisioner';
import { provisionPiShim } from './services/agentpanel/PiShimProvisioner';
import {
...
export async function startStandaloneServer(
  ...
  composition.context.lifecycle.startPromise ??= startRuntime({
    ...
    ensureAgentPanelShim: async () => {
      await provisionAgentPanelShim({
        hermitHome: environment.hermitHome,
        packageRoot: environment.repoRoot,
        version: environment.version,
      });
      if (process.env.AGENTPANEL_PACKAGED_DESKTOP === '1') {
    ...
    ccConnectEnabled: environment.ccConnectEnabled,
    markBridgeBinaryCheck,
    markBridgeLaunch,
    markBridgeDisabled,
    processTarget,
