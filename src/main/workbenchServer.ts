...
import {
  ...
  registerGitHubDeliveryRoutes,
} from '@features/github-delivery/main';
import {
...
import { setCapabilityPackLocalSource, setSkillsWatcherEmitter } from './ipc/extensions';
...
import { registerExternalChannelRoutes } from './routes/externalChannelRoutes';
...
import { resolveLoopbackWorkbenchUrl } from './services/agentpanel/workbenchRuntimeEnv';
import { stopTelemetry } from './services/session-intelligence/UsageTelemetryService';
...
async function createWorkbenchServerUncached(
  ...
  const githubDelivery = new GitHubDeliveryService(environment.hermitHome);
  ...
  const teamTaskRouteDependencies = {
    ...
    dispatchTask: async (
      ...
      const cliPath = `${hermitHome}/bin/agentpanel`;
      ...
      const text = [
        ...
        '',
        '请使用内置 AgentPanel 更新任务状态，不要使用其他任务系统：',
  ...
  registerHarnessRoutes(app, {
    ...
    listProjects: () => cc.listProjects(),
    packagedDesktop: process.env.AGENTPANEL_PACKAGED_DESKTOP === '1',
  });
  ...
  registerExternalChannelRoutes(app, {
    ...
    enabledAtStartup: environment.ccConnectEnabled,
    getRuntimeStatus: getRuntimeReadiness,
  });
  registerGitHubDeliveryRoutes(app, { service: githubDelivery });
  // Usage collection, manual scans, and report/export endpoints are intentionally
  // unavailable in the local-first desktop product. Team/task operation does not
  // depend on session telemetry.
