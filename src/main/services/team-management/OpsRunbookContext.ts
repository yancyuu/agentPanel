/**
 * Compact AgentPanel operations context injected into managed agent instructions.
 *
 * Keep this short: agents need the stable runbook boundary and pointers, not the
 * full public guide copied into every session.
 */

export const HERMIT_OPS_GUIDE_URL = 'https://yancyuu.github.io/agentpanel/';

const OPS_CONTEXT_BEGIN = '<!-- hermit:ops-runbook-context:start -->';
const OPS_CONTEXT_END = '<!-- hermit:ops-runbook-context:end -->';

const OPS_RUNBOOK_CONTEXT = `## AgentPanel Ops Runbook Context

Local canonical docs: README.md, docs/README.md, docs/TEAM_COLLABORATION_MODEL.md
Bundled diagnostics guide: the versioned AgentPanel guide written to the diagnostics CLAUDE.md.

AgentPanel is a local-first digital-worker Workbench. Humans create long-running work from
the global “创建任务” inbox, review delivery in “待审核”, and use “调教” from an Agent page
for short behavior adjustments. Agents maintain task state through the bundled AgentPanel
