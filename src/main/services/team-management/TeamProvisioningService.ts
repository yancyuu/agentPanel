...
export class TeamProvisioningService {
   ...
   * 目标团队通过 AgentPanel 任务总线认领、评论、澄清并提交任务。
   */
  async dispatchTask(sourceTeamSlug: string, task: Task): Promise<void> {
    ...
    const cliEntry = path.join(
      hermitHome,
      'bin',
      process.platform === 'win32' ? 'agentpanel.cmd' : 'agentpanel'
    );
    ...
    const message = [
      ...
      ``,
      `请通过 AgentPanel 任务总线处理，不要使用 MCP、Skills 或运行时自带任务系统：`,
...
Cross-team collaboration is handled through AgentPanel's task bus. AgentPanel task state is the single source of truth.
...
- List visible tasks: \`agentpanel --port ${process.env.PORT ?? '5680'} tasks list --team ${teamSlug}\`
- Claim a task before work: \`agentpanel --port ${process.env.PORT ?? '5680'} tasks claim --team ${teamSlug} --id <task-id>\`
- Request clarification: \`agentpanel --port ${process.env.PORT ?? '5680'} tasks clarify --team ${teamSlug} --id <task-id> --target user\`
- Submit the result: \`agentpanel --port ${process.env.PORT ?? '5680'} tasks complete --team ${teamSlug} --id <task-id> --result "<result>"\`

Do not call cross-team dispatch APIs yourself and do not invent task IDs. Use only task IDs returned by the CLI or supplied by AgentPanel.
