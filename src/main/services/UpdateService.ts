...
// Resolve the agentpanel package root by walking up to the package.json whose
// name is @yancyyu/agentpanel. Robust to layout (source src/main/ vs precompiled
// bundle dist/ vs future SEA exe) -- a hardcoded `../../..` only worked in
// source and broke in the bundle.
function findAgentPanelRoot(startDir: string): string {
  let dir = startDir;
  for (let i = 0; i < 12; i++) {
    try {
      if (
...
const REPO_ROOT = findAgentPanelRoot(__dirname);
...
const GITHUB_REPO = 'yancyuu/agentpanel';
