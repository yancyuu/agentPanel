 ...
 * an externally managed bridge. (agentpanel brands this component as "bridge"
 ...
 * path (agentpanel's own mirror-based fetcher), which is the reliable fix.
 */
export function resolvePackagedBridgeBinary(
  platform: NodeJS.Platform = process.platform,
  arch: NodeJS.Architecture = process.arch,
  packageRoot = process.env.AGENTPANEL_PACKAGE_ROOT?.trim()
): string | null {
  if (!packageRoot) return null;
  const targetOs = platform === 'win32' ? 'windows' : platform;
  const cpu = arch === 'x64' ? 'amd64' : arch;
  const binaryName = resolveHermitBridgeBinaryName(platform, arch);
  if (
    !binaryName ||
...
function resolveHermitBridgeRunner(): string | null {
  // Packaged desktop releases carry one reviewed, target-specific binary. Prefer
  // it to node_modules so a cross-architecture build never launches the builder
  // host's optionalDependency executable.
  const packagedBinary = resolvePackagedBridgeBinary();
  if (packagedBinary) return packagedBinary;

  // Never fall back to cc-connect/run.js: it can trigger an unmanaged download
  // and console window on Windows. An absent binary falls through to the
  // controlled fetcher, which can copy the bundled target or report a failure.
  try {
 ...
 * e.g. for users who installed agentpanel before the install.js mirror patch
...
export class HermitBridgeLauncher {
  ...
  stop(): void {
    ...
    try {
      ...
      log.info('stopped cc-connect launched by AgentPanel');
    } catch (err) {
