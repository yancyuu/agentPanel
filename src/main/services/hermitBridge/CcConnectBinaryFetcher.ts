 ...
 *   hermit-bridge can't start. For users who already installed agentpanel
 ...
 *   cannot find a usable runner (or the runner finds no binary), agentpanel
 ...
 *   agentpanel or touch GitHub directly.
 *
...
/** Resolve the cc-connect version agentpanel expects, from package.json. */
function resolveExpectedVersion(): string | null {
  ...
  // 2. Read agentpanel's own package.json (works both published and from source).
  try {
    const candidates = [
      () => require.resolve('@yancyyu/agentpanel/package.json'),
      ...
      () => {
        ...
        for (let i = 0; i < 8 && dir !== path.dirname(dir); i++) {
          ...
          if (fs.existsSync(candidate)) {
            ...
            if (pkg.name === '@yancyyu/agentpanel') return candidate;
          }
          dir = path.dirname(dir);
        }
        throw new Error('agentpanel package.json not found');
      },
    ];
...
async function fetchToBuffer(
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = request(
      url,
      { headers: { 'User-Agent': 'agentpanel-cc-connect-fetcher' }, timeout: 60_000 },
      (res) => {
...
export async function ensureCcConnectBinary(
  ...
  const packageRoot = process.env.AGENTPANEL_PACKAGE_ROOT?.trim();
