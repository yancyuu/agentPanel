...
export class LoopAssetsScannerService {
  ...
  private async scanSubagents(
    ...
    if (memberCount > 0) {
      details.push(`${memberCount} AgentPanel team members`);
      sources.push({ label: 'AgentPanel team members', scope: 'team', kind: 'team-members' });
    }
  ...
  private async scanState(
    ...
    if (input.taskCount != null && input.taskCount > 0) {
      details.push(`${input.taskCount} AgentPanel tasks`);
      sources.push({
        label: 'AgentPanel task board',
        path: input.teamRoot,
        scope: 'team',
        kind: 'task-board',
      });
