export const meta = {
  ...
  description:
    'Read-only inventory audit for AgentPanel and Claude Code workflows, commands, skills, cron, hooks, and generated dynamic workflow scripts.',
  category: 'compliance',
  safety: 'audit',
  order: 115,
  phases: [
...
const inventory = await agent(
  `只读盘点 AgentPanel / Claude Code / 项目自动化资产。

目标项目：${options.projectRoot}
AgentPanel 预设 workflow 目录：${options.hermitWorkflowDir}
...
const classification = await agent(
  `根据以下只读清单，把自动化资产分类并判断是否适合沉淀为 AgentPanel 预设动态 workflow。
...
输出：
1. 已安装/已沉淀能力。
2. 值得产品化为 AgentPanel 预设动态 workflow 的候选，含建议文件名、用途、安全等级和理由。
3. 重复或冲突入口。
4. 高风险自动化和必须人工确认的边界。
5. 不建议迁移的资产及原因。
