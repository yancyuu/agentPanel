...
export function markBridgeBinaryCheck(state: BridgeBinaryState): void {
  ...
  if (state.status === 'ok') {
    ...
    log.info('cc-connect external-channel integration is disabled');
  }
}
...
/** Mark the optional external-channel integration as intentionally inactive. */
export function markBridgeDisabled(): void {
  bridgeBinary = { status: 'disabled' };
  bridgeLaunch = { status: 'disabled' };
  touch();
  log.info('cc-connect external-channel integration is disabled');
}
...
export function getRuntimeReadiness(): RuntimeReadiness {
  const status: RuntimeReadiness['status'] =
    bridgeBinary.status === 'disabled' || bridgeLaunch.status === 'disabled'
      ? 'disabled'
      : bridgeBinary.status === 'ok' && bridgeLaunch.status === 'running'
        ? 'ok'
        : 'degraded';
