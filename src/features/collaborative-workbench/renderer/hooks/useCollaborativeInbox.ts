...
import type { TeamMessagesCacheEntry, TeamSlice } from '@renderer/store/slices/teamSlice';
...
export interface CollaborativeInboxState {
  ...
  sendTeamMessage: TeamSlice['sendTeamMessage'];
  setTaskNeedsClarification: TeamSlice['setTaskNeedsClarification'];
}
...
export function useCollaborativeInbox(): CollaborativeInboxState {
  const {
    ...
    useShallow((state) => ({
      ...
      setTaskNeedsClarification: state.setTaskNeedsClarification,
    }))
  );
