...
const ALL_AGENT_FOLDERS_SCOPE = `必须覆盖所有已管理且未删除的数字员工，不能抽样。先通过 AgentPanel 团队清单和 ~/.hermit/teams/*/team.json 建立员工清单，再为每个员工解析：
...
const FULL_SCAN_ACTION: DiagnosticAction = {
  ...
  prompt: `请执行一次 AgentPanel 核心健康检查，并用普通用户能理解的中文输出结果。

只检查三项，不要扩展范围：
1. 每个数字员工的运行状态：团队目录与工作目录是否可访问、运行时是否在线；不可访问的列出原因。
2. 全局任务积压：长期无进展、等待用户补充、无人负责、交付后待审核过久的任务。
3. Workbench、内置 AgentPanel、任务总线、运行时和外部连接是否可用。

先通过 AgentPanel 团队清单和 ~/.hermit/teams/*/team.json 建立员工清单，再逐项给出“正常 / 需关注 / 建议处理”三组汇总；每条说明发生了什么和用户下一步应点哪里。不要逐字读取员工文档内容，不要读取或输出敏感正文。${READ_ONLY_BOUNDARY}`,
};
...
对比每个成员实际目录中的 CLAUDE.md、AGENTS.md、角色说明、团队协作指令和当前 AgentPanel 任务协议，找出重复、互相矛盾、过期路径、旧品牌说明和成员之间不一致的规则。输出员工总数、已扫描数、跳过项及原因，并按团队和成员列出风险等级、证据类型与建议保留的唯一事实来源。不要读取或输出敏感正文。${READ_ONLY_BOUNDARY}`,
  },
  {
    id: 'folder-hygiene',
    title: '临时文件扫描',
...
逐个员工目录检查是否存在过多临时文件、日志、截图、缓存、重复报告、陈旧构建产物、脏 worktree 或长期未使用的 agent workspace。输出员工总数、已扫描数、跳过项及原因；只列出可安全清理候选、占用或数量级、风险和需要人工确认的项目，不要实际删除。${READ_ONLY_BOUNDARY}`,
  },
  {
    id: 'runtime-health',
    title: '运行环境检查',
    description: '检查 Workbench、AgentPanel、运行时和外部连接。',
    icon: Activity,
    prompt: `请执行 AgentPanel 运行环境只读检查。

检查 Workbench 当前地址、内置 AgentPanel、任务总线、Claude CLI、hermit-bridge 兼容 sidecar、所有数字员工的运行目录和外部渠道连接是否可用。不要要求用户全局安装旧 openhermit/hermit，不要修改 Shell PATH。用“正常 / 异常 / 下一步”输出。${READ_ONLY_BOUNDARY}`,
  },
  {
    id: 'task-health',
    title: '任务积压检查',
    ...
    icon: ClipboardCheck,
    prompt: `请只读检查 AgentPanel 全局任务状态。
              ...
              {cleanupScan ? (
                <div className="space-y-4">
                  ...
                  <p className="text-[11px] leading-5 text-[var(--color-text-muted)]">
                    只会列出白名单中的缓存、旧日志和 AgentPanel 临时文件；不会清理项目源码、Git
