...
vi.mock('./sections', () => ({
  ...
  PluginsSection: (props: Record<string, unknown>) => {
    testState.mountedProps.plugins = props;
    return <div data-testid="plugins-section">插件内容</div>;
  },
...
describe('SettingsView navigation and layout', () => {
  beforeEach(() => {
    ...
    testState.mountedProps.plugins = undefined;
  ...
  it('renders plugin management and switches the conditionally mounted content', async () => {
    ...
    await clickCategory(host, '插件');
    expect(host.querySelector('[data-testid="plugins-section"]')).not.toBeNull();
    expect(host.querySelector('[data-testid="harness-section"]')).toBeNull();

    await clickCategory(host, '高级');
    expect(host.querySelector('[data-testid="advanced-section"]')).not.toBeNull();
    expect(host.querySelector('[data-testid="plugins-section"]')).toBeNull();
  ...
  it('keeps every tab control target stable while only mounting the active section', async () => {
    ...
    for (const section of ['harness', 'plugins', 'advanced']) {
