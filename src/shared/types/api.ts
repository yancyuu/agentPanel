...
export interface ElectronAPI extends RecentProjectsElectronApi {
  ...
  // Opt-in external channel integrations. A pending state requires app relaunch.
  externalChannels: {
    get: () => Promise<{
      ccConnect: {
        enabled: boolean;
    ...
    setCcConnectEnabled: (enabled: boolean) => Promise<{
      ccConnect: {
        enabled: boolean;
        active: boolean;
        restartRequired: boolean;
  ...
  // GitHub is an optional delivery enhancement and uses the local Git credential helper / SSH key.
  githubDelivery: {
    ...
    deleteBinding: (agentName: string) => Promise<void>;
    publish: (request: { teamName: string; taskId: string; agentName: string }) => Promise<{
      repository: string;
      branch: string;
      path: string;
      commit: string;
    ...
    downloadArchive: (teamName: string, taskId: string) => Promise<Blob>;
  };
